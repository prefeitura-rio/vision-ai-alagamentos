...
ffmpeg -re -stream_loop -1 -i ${DATA_PATH}/video-1.mp4 -vcodec libx264 -f rtsp rtsp://${STREAM_SERVER}:8554/video-1 || exit 1 &
ffmpeg -re -stream_loop -1 -i ${DATA_PATH}/video-1.mp4 -vcodec libx265 -f rtsp rtsp://${STREAM_SERVER}:8554/video-2 || exit 1 &
wait
